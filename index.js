...
function dbWriteBlockStats() {
       ...
       poolName: addr2name[tx.vout[0].scriptPubKey.addresses[0]] == null ?

           tx.vout[0].scriptPubKey.addresses[0] : addr2name[tx.vout[0].scriptPubKey.addresses[0]],
                                                },
                                        }], {
                                                precision: 's',
                                        }).catch(err => {
...
function dbWritePoolStats() {
        ...
        request(options).then(function(hp) {
                db.writePoints([{
                        ...
                        fields: {
                                poolHps: hp.hashrate,
                                workers: hp.workers,                                sharesThisRound: hp.shares_this_round,
